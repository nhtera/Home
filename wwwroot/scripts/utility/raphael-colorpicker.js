...
(function (Raphael) {
    ...
    var doc = document, win = window,
        ColorPicker = function (x, y, size, initcolor, element) {
            ...
            t.disc.drag(function (dx, dy, x, y) {
                ...
                var o = t.position(t.el);
                t.hsOnTheMove = true;
                t.setHS(x - t.x - o.x, y - t.y - o.y);
            }, function () {
                t.hsOnTheMove = false;
            });
    ...
    ColorPicker.prototype.docOnMove = function (dx, dy, x, y) {
        var o = this.position(this.el);
        if (this.hsOnTheMove) {
            this.setHS(x - this.x - o.x, y - this.y - o.y);
        }
        if (this.bOnTheMove) {
            this.setB(x - this.x - o.x);
        }
    };
    ColorPicker.prototype.position = function (myelem) {
        //cross-browser compatible element x & y position & width & height
        var x = 0, y = 0, w = 0, h = 0, elem = myelem;
